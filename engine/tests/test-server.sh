...
sleep 5s
$LAUNCHER /usr/bin/time -p $MABOSS_CLIENT --host localhost --port 7777 cellcycle/cellcycle.bnd -c cellcycle/cellcycle_runcfg.cfg -c cellcycle/cellcycle_runcfg-thread_1.cfg -o tmp/Cell_cycle_thread_1
if [ $? != 0 ]; then exit 1; fi
