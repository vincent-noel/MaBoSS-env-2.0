...
if [ $? != 0 ]; then exit 1; fi
diff --strip-trailing-cr refer/Cell_cycle_thread_1_probtraj.csv /tmp/
check_file "projtraj"

diff --strip-trailing-cr refer/Cell_cycle_thread_1_statdist.csv /tmp/ # || echo '**** error test #1.b (non regression) ****'
check_file "statdist"
